...
################################################################################
## INGESTION MICRODADOS DADOS RAIS  - ARQUIVOS 2010 a 2017                    ##
## ********        GOING DATA WAY     ********                                ##
...
#1download.file('../7zip/7za.exe', tf7z, mode='wb')
...
# URL exemplo ftp://ftp.mtps.gov.br/pdet/microdados/RAIS/2017/AC2010.7z

# Arquivos entre 2018 e 2019 seguem uma nova nomenclatura
regionlst <- list('AC','AL','AM','AP','BA','CE','DF','ES','GO','MA','MG','MS','MT','PA','PB','PE','PI','PR','RJ',
...
for(ano in 2010:2017){
  for(region in regionlst){
    ...
    filelist <- c(filelist, paste0("ftp://ftp.mtps.gov.br/pdet/microdados/RAIS/",ano,"/",paste0(region,ano),".7z")) # Obter endereço de cada arquivo
  }
}
#print(filelist)
...
for(file in filelist){
  ...
  dfname <- txtfile
  txtfile = paste0(getwd(),'/',txtfile,'.Rda') # Substituir o getwd() para escolher a pasta onde sera salvo os arquivos rda.
